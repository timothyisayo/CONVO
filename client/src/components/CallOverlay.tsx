...
type Request = { conversationId: string; calleeId?: string | null; callType: "voice" | "video"; calleeName?: string; isGroup?: boolean };
type Props = { supabase: SupabaseClient | null; userId: string; displayName: string };

export function CallOverlay({ supabase, userId, displayName }: Props) {
  ...
  useEffect(() => {
    ...
    channel.on("broadcast", { event: "incoming" }, ({ payload }: any) => { if (payload?.call && !callRef.current && !incomingRef.current) { participantIdsRef.current = Array.isArray(payload.participantIds) ? payload.participantIds : []; setIncomingState(payload.call); setPeerName(typeof payload.callerName === "string" ? payload.callerName : ""); setStatus("ringing"); window.dispatchEvent(new CustomEvent("convo:incoming-call", { detail: { calleeName: typeof payload.callerName === "string" ? payload.callerName : "Someone" } })); } });
    ...
    const handler = (event: Event) => { const request = (event as CustomEvent<Request>).detail; void (async () => { try { const result = await startProviderCall(supabase, request); participantIdsRef.current = result.participantIds || []; setPeerName(request.calleeName || "MTU student"); setCallState(result.call); setStatus("calling"); for (const recipientId of participantIdsRef.current) { if (recipientId !== userId) await sendSignal(recipientId, "incoming", { call: result.call, callerName: displayName, groupName: request.isGroup ? request.calleeName : undefined, participantIds: participantIdsRef.current }); } await connectRoom(result.call, false); } catch (e) { setError(e instanceof Error ? e.message : "This student cannot receive calls right now."); setStatus("failed"); } })(); };
