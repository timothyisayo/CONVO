...
type Props = {
  ...
  onSearchStudents?: (query: string) => Promise<{ data: Array<{ id: string; display_name: string; student_id: string; is_self?: boolean; programme: string | null; department: string | null; level: string | null; status_text: string | null }>; error: { message: string } | null }>;
...
export function ConvoDashboard({ currentUserId = "", displayName, legalName = "", major, avatarUrl, studentId = "", level = "", department = "", programme, bio = "", profileVisibility = { programme: true, college: true, level: true, bio: true }, groups, posts, joinedGroupIds,   onJoinGroup, onSearchStudents, onSendConnectionRequest, onCancelConnectionRequest, onStartDirectConversation, onCreateGroupConversation, onUpdateGroupImage, onLoadGroupMembers, onAddGroupMembers, onSetGroupMemberRole, onRemoveGroupMember, onCreateGroupInvite, onRotateGroupInvite, onLoadGroupPermissions, onSetGroupPermissions, onJoinGroupInvite, onLoadGroupJoinRequests, onReviewGroupJoinRequest, onSendMessage, onEditMessage, onDeleteMessage, onLoadMessageInteractions, onToggleMessageReaction, onToggleSavedMessage, onTogglePinnedMessage, onSetConversationPreference, onSetConversationRailState, onLoadPrivacySettings, onSetPrivacySettings, onLoadConversationNotificationPreference, onSetConversationNotificationPreference, notificationsEnabled = false, onSetNotificationsEnabled, onLoadConversationAppearance, onSetConversationAppearance, onLoadSavedMessages, onSearchConversationMessages, onCreateGroupPoll, onUpdateGroupPoll, onCloseGroupPoll, onDeleteGroupPoll, onLoadGroupEvents, onSetGroupEventResponse, onCreateGroupEvent, onCancelGroupEvent, onUpdateGroupEvent, onDeleteGroupEvent, onLoadGroupNotes, onUpdateGroupNote, onCreateGroupNote, onLoadGroupAnnouncements, onCreateGroupAnnouncement, onUpdateGroupAnnouncement, onDeleteGroupAnnouncement, onLoadGroupPolls, onVoteOnGroupPoll, onCreateGroupTask, onUpdateGroupTask, onDeleteGroupTask, onLoadGroupTasks, onSetGroupTaskCompleted, onSubscribeToGroupActivity, onLoadConnectionRequests, onSubscribeToConnectionRequests, onAcceptConnectionRequest, onBlockStudent, onLoadBlockedStudents, onUnblockStudent, onReportStudent, onTouchLastSeen, onSearchGroups, onRequestGroupJoin, onEndGroup, onSetGroupPrivate, onDeleteGroupMessage, onLoadSharedFiles, onLoadConversations,   onLoadMessages, onMarkConversationRead, onSubscribeToPublicProfiles, onSubscribeToMessages, onSubscribeToAllMessages, onSubscribeToConversation, onExit, onLogout, onUpdateProfile, onUpdateAvatar, onUpdatePrivacy, onAskAssistant, vaultClient, isExiting = false, isEntering = false }: Props) {
  ...
  const [groupMembers, setGroupMembers] = React.useState<Array<{ user_id: string; display_name: string; student_id: string; group_role: "owner" | "admin" | "member" }>>([]);
  ...
  React.useEffect(() => {
    ...
    const requestTimer = window.setTimeout(() => { void onSearchStudents(requestedDirectoryQuery).then((result) => {
      ...
      else setLiveStudents(result.data.map((student, index) => ({ id: student.id, isSelf: Boolean(student.is_self), name: student.display_name, initials: student.display_name.slice(0, 2).toUpperCase(), programme: student.programme || "", department: student.department || "", level: student.level || "", tone: ["rose", "sage", "butter", "apricot"][index % 4], status: student.status_text || "", mutual: "" })));
      setDirectoryLoading(false);
    }).catch(() => {
      if (!active || settled) return;
      settled = true;
  ...
  React.useEffect(() => {
    ...
    void onSearchStudents(publicId).then((result) => {
      ...
      const students = result.data.map((student, index) => ({ id: student.id, isSelf: Boolean(student.is_self), name: student.display_name, initials: student.display_name.slice(0, 2).toUpperCase(), programme: student.programme || "", department: student.department || "", level: student.level || "", tone: ["rose", "sage", "butter", "apricot"][index % 4], status: student.status_text || "", mutual: "" }));
  ...
  React.useEffect(() => {
    if (activeView !== "groups" || !onSearchGroups) return;
    let cancelled = false;
  ...
  React.useEffect(() => {
    ...
    const unsubscribe = onSubscribeToAllMessagesRef.current((message) => {
      if (String(message.sender_id || "") === currentUserId || typeof Notification === "undefined" || Notification.permission !== "granted") return;
      const conversation = liveConversationsRef.current.find((item) => item.id === String(message.conversation_id || ""));
      new Notification(conversation?.name || "New Convo message", { body: String(message.body || "New message") });
    });
  ...
  const shareMyProfile = async () => { if (!studentId) { toast("Complete your profile before sharing your public student card."); return; } const link = `${window.location.origin}/?profile=${encodeURIComponent(studentId)}`; try { await navigator.clipboard?.writeText(link); } catch { /* The message draft remains a usable fallback. */ } if (activeConversation) { setMessageDraft(`My Convo profile: ${link}`); composerTextareaRef.current?.focus(); toast.success("Profile link ready to send"); } else toast.success("Public profile link copied", { description: "Share it with an MTU student using Convo." }); setShowThreadActions(false); };
  const archiveCurrentConversation = async () => { if (!activeConversation || !onSetConversationPreference) { toast("Conversation hiding will activate after the messaging update is applied."); return; } const result = await onSetConversationPreference(activeConversation.id, null, true); if (!result.ok) { toast.error("Couldn’t hide conversation", { description: result.error || "Please try again." }); return; } setLiveConversations((current) => current.filter((conversation) => conversation.id !== activeConversation.id)); setSelectedConversationId(""); setShowThreadActions(false); toast.success("Conversation hidden", { description: "It is hidden only for you." }); };
  ...
  const openGroupPermissionsDialog = () => {
    ...
    void onLoadGroupPermissions(activeConversation.id).then((result) => {
      ...
      const host = document.createElement("div"); host.className = "convo-private-label-backdrop";
      host.innerHTML = `<form class="convo-private-label-dialog convo-group-permissions-dialog" aria-label="Group permissions"><button type="button" aria-label="Close group permissions" class="convo-private-label-close">×</button><span class="eyebrow dark">Owner controls</span><h2>Group<br><em>permissions.</em></h2><p>Decide how this private MTU group works. Admins always retain moderation access.</p><label class="permission-toggle"><input name="allow_messages" type="checkbox" ${result.data.allow_member_messages ? "checked" : ""}><span><strong>Members can send messages</strong><small>Turn this off for an admins-only announcement group.</small></span></label><label class="permission-toggle"><input name="allow_invites" type="checkbox" ${result.data.allow_member_invites ? "checked" : ""}><span><strong>Members can share invite links</strong><small>When off, only admins can create invitations.</small></span></label><label class="permission-toggle"><input name="require_approval" type="checkbox" ${result.data.require_join_approval ? "checked" : ""}><span><strong>Approve new members</strong><small>New links require an admin review before entry.</small></span></label><div><button type="button" class="outline-button">Cancel</button><button type="submit" class="primary-button">Save permissions</button></div></form>`;
  ...
  const openGroupPollsDialog = () => {
    ...
    void onLoadGroupPolls(activeConversation.id).then((loaded) => {
      ...
      const grouped = Array.from(loaded.data.reduce((collection, row) => { const existing = collection.get(row.poll_id) || { ...row, options: [] as Array<typeof row> }; existing.options.push(row); collection.set(row.poll_id, existing); return collection; }, new Map<string, { poll_id: string; question: string; closes_at?: string | null; is_closed: boolean; anonymous_voters: boolean; options: Array<(typeof loaded.data)[number]> }>()).values());
      const pollMarkup = grouped.length ? grouped.map((poll) => `<article class="convo-poll-card"><strong>${poll.question.replace(/</g, "&lt;")}</strong><small>${poll.is_closed ? "Voting closed" : poll.closes_at ? `Closes ${new Date(poll.closes_at).toLocaleString()}` : "No deadline"}${poll.anonymous_voters ? " · private voters" : ""}</small><div>${poll.options.map((option) => `<button type="button" ${poll.is_closed ? "disabled" : ""} data-poll-id="${poll.poll_id}" data-option-id="${option.option_id}" class="${option.selected_by_me ? "is-selected" : ""}"><span>${option.option_label.replace(/</g, "&lt;")}</span><b>${option.vote_count}</b></button>`).join("")}</div></article>`).join("") : "<p class=\"convo-utility-empty\">No group polls yet. Create one below.</p>";
      const host = document.createElement("div"); host.className = "convo-private-label-backdrop";
      host.innerHTML = `<section class="convo-private-label-dialog convo-utility-dialog convo-polls-dialog" role="dialog" aria-modal="true" aria-label="Group polls"><button type="button" aria-label="Close group polls" class="convo-private-label-close">×</button><span class="eyebrow dark">Group decisions</span><h2>Polls.</h2><div class="convo-poll-list">${pollMarkup}</div><form class="convo-utility-form"><label>Question<input name="question" maxlength="240" required></label><label>Option one<input name="option_one" maxlength="100" required></label><label>Option two<input name="option_two" maxlength="100" required></label><label>Closes at <input name="closes_at" type="datetime-local"></label><label class="convo-checkbox-row"><input name="anonymous" type="checkbox"> Hide voter identities</label><button type="submit" class="primary-button">Create poll</button></form></section>`;
      const close = () => host.remove(); host.addEventListener("click", (event) => { if (event.target === host) close(); }); host.querySelector(".convo-private-label-close")?.addEventListener("click", close);
      ...
      (host.querySelector("form") as HTMLFormElement).addEventListener("submit", (event) => { event.preventDefault(); const values = new FormData(event.currentTarget as HTMLFormElement); const deadline = String(values.get("closes_at") || ""); const closesAt = deadline ? new Date(deadline).toISOString() : null; void onCreateGroupPoll(activeConversation.id, String(values.get("question") || ""), [String(values.get("option_one") || ""), String(values.get("option_two") || "")], closesAt, values.has("anonymous")).then((result) => { if (result.error) toast.error("Couldn’t create poll", { description: result.error }); else { toast.success("Poll created"); close(); } }); });
      document.body.appendChild(host);
    });
  };
  ...
  const openGroupInviteQrDialog = async () => {
    ...
    try {
      const qrDataUrl = await QRCode.toDataURL(link, { width: 240, margin: 1, color: { dark: "#4a2c36", light: "#fffaf4" } });
      const host = document.createElement("div"); host.className = "convo-private-label-backdrop";
  ...
  const openConversationProfilePanel = () => {
    ...
    const directActions = `<button type="button" data-profile-share class="profile-sheet-primary">Copy my public profile link</button><div class="profile-sheet-utilities"><button type="button" data-conversation-search>Search this chat</button><button type="button" data-conversation-notifications>Notifications</button><button type="button" data-conversation-appearance>Appearance</button><button type="button" data-profile-rename>Name this chat</button><button type="button" data-profile-hide>Hide from inbox</button></div>${activeConversation.counterpartId && (onReportStudent || onBlockStudent) ? `<details class="profile-sheet-safety"><summary>Safety & privacy</summary><div>${onReportStudent ? '<button type="button" data-profile-report>Report student</button>' : ""}${onBlockStudent ? '<button type="button" data-profile-block class="is-danger">Block student</button>' : ""}</div></details>` : ""}`;
    ...
    host.innerHTML = `<section class="convo-private-label-dialog convo-conversation-profile ${isGroup && groupImageUrl ? "has-group-image" : ""}" role="dialog" aria-modal="true" aria-label="${isGroup ? "Group profile" : "Student profile"}">${groupImageCover}<button type="button" aria-label="Close conversation profile" class="convo-private-label-close">×</button><span class="eyebrow dark">${isGroup ? "MTU group profile" : "Public student profile"}</span><div class="conversation-profile-avatar">${profileAvatar}</div><h2>${safeName}</h2><p>${isGroup ? `${groupMembers.length} member${groupMembers.length === 1 ? "" : "s"} · ${ownRole ? `${ownRole} access` : "member access"}` : "Only this student’s public nickname and chosen academic details are visible here."}</p><div class="conversation-profile-actions">${isGroup ? groupActions : directActions}</div></section>`;
    const close = () => host.remove();
    ...
    host.querySelector<HTMLButtonElement>("[data-profile-share]")?.addEventListener("click", () => { void shareMyProfile(); close(); });
    host.querySelector<HTMLButtonElement>("[data-profile-rename]")?.addEventListener("click", () => { close(); openPrivateLabelDialog(); });
    host.querySelector<HTMLButtonElement>("[data-profile-hide]")?.addEventListener("click", () => { close(); void archiveCurrentConversation(); });
    ...
    host.querySelector<HTMLButtonElement>("[data-group-end]")?.addEventListener("click", () => { if (!onEndGroup || !activeConversation) return; if (!window.confirm("End this group for all members? This cannot be undone.")) return; void onEndGroup(activeConversation.id).then((result) => { if (!result.ok) { toast.error("Couldn’t end group", { description: result.error || "Only the group owner can end this group." }); return; } closeActiveThread(); setLiveConversations((current) => current.filter((conversation) => conversation.id !== activeConversation.id)); setActiveView("groups"); toast.success("Group ended"); }); });
  ...
  React.useEffect(() => {
    ...
    if (activeConversation && (activeConversation.kind === "group" || activeConversation.counterpartId)) {
      ...
      const callDetail = { conversationId: activeConversation.id, calleeId: activeConversation.kind === "group" ? null : activeConversation.counterpartId, callType: "voice" as const, calleeName: callName, isGroup: activeConversation.kind === "group" };
      const voiceCall = document.createElement("button");
      voiceCall.type = "button"; voiceCall.className = "thread-call-action thread-call-action-voice"; voiceCall.setAttribute("aria-label", `Voice call ${callName}`); voiceCall.innerHTML = '<span class="thread-call-action-icon" aria-hidden="true">☎</span><span>Voice</span>'; voiceCall.addEventListener("click", () => window.dispatchEvent(new CustomEvent("convo:start-call", { detail: callDetail })));
      const videoCall = document.createElement("button");
      videoCall.type = "button"; videoCall.className = "thread-call-action thread-call-action-video"; videoCall.setAttribute("aria-label", `Video call ${callName}`); videoCall.innerHTML = '<span class="thread-call-action-icon" aria-hidden="true">▣</span><span>Video</span>'; videoCall.addEventListener("click", () => window.dispatchEvent(new CustomEvent("convo:start-call", { detail: { ...callDetail, callType: "video" } })));
      actions.append(voiceCall, videoCall);
    }
    if (activeConversation) {
      const profileTrigger = document.createElement("button"); profileTrigger.type = "button"; profileTrigger.className = "thread-profile-trigger"; profileTrigger.setAttribute("aria-label", activeConversation.kind === "group" ? "Open group profile" : "Open student profile"); profileTrigger.textContent = activeConversation.kind === "group" ? "Group info" : "Profile"; profileTrigger.addEventListener("click", openConversationProfilePanel); actions.appendChild(profileTrigger);
    }
    header.querySelector(".thread-header-actions")?.remove();
    header.appendChild(actions);
    return () => actions.remove();
  }, [activeConversation?.counterpartId, activeConversation?.id, activeConversation?.kind, activeConversation?.name, activeView, groupMembers, onBlockStudent, onReportStudent, onEndGroup, selectedConversationId]);
  React.useEffect(() => {
  ...
  React.useEffect(() => {
    ...
    const trigger = document.createElement("button"); trigger.type = "button"; trigger.className = "text-link convo-profile-share-trigger"; trigger.setAttribute("aria-label", "Copy public profile link"); trigger.textContent = "Share"; trigger.addEventListener("click", () => { void shareMyProfile(); }); row.append(trigger);
    return () => trigger.remove();
  ...
  React.useEffect(() => {
    ...
    threadMessages.forEach((message) => {
      ...
      more.addEventListener("click", () => { actions.querySelector(".compact-message-menu")?.remove(); const menu = document.createElement("span"); menu.className = "compact-message-menu"; const canManage = message.sender_id === currentUserId && !message.deleted_at && Date.now() - Date.parse(message.created_at) < 15 * 60 * 1000; const entries: Array<[string, () => void]> = [["Reply", () => { setReplyingTo(message); composerTextareaRef.current?.focus(); }], ["Forward", () => openForwardMessage(message)], ["Copy", () => void copyMessageText(message)], [message.attachment_url ? "Save to device" : (messageInteractions[message.id]?.saved ? "Unsave" : "Save"), () => message.attachment_url ? void saveAttachmentToDevice(message.attachment_url, message.attachment_path?.split("/").pop() || "convo-attachment") : void toggleSaved(message.id)], ["Info", () => showMessageInfo(message)]]; if (canManage) entries.push(["Edit", () => startEditingMessage(message)], ["Delete", () => void removeMessage(message.id)]); entries.forEach(([label, handler]) => { const option = document.createElement("button"); option.type = "button"; option.textContent = label; option.addEventListener("click", () => { handler(); menu.remove(); }); menu.append(option); }); actions.append(menu); });
      actions.append(react, more); bubble.appendChild(actions);
    });
  ...
  React.useEffect(() => {
    ...
    const panel = document.querySelector<HTMLElement>(".thread-panel");
    if (!panel) return;
    panel.querySelectorAll(".message-reply-context,.message-reaction-summary").forEach((node) => node.remove());
    threadMessages.forEach((message) => {
      const bubble = panel.querySelector<HTMLElement>(`[data-message-id="${message.id}"]`);
      if (!bubble || message.deleted_at) return;
  ...
  }, [activeView, currentUserId, messageInteractions, selectedConversationId, threadMessages]);
  React.useEffect(() => {
    if (activeView !== "messages") return;
  ...
  React.useEffect(() => {
    ...
    const video = document.createElement("button"); video.type = "button"; video.className = "composer-video-action"; video.setAttribute("aria-label", "Choose video attachment"); video.innerHTML = '<span class="composer-media-glyph" aria-hidden="true">▷</span><span>Video</span>'; video.addEventListener("click", openVideo);
    const record = document.createElement("button"); record.type = "button"; record.className = "composer-video-action"; record.setAttribute("aria-label", "Record video attachment"); record.innerHTML = '<span class="composer-media-glyph" aria-hidden="true">◉</span><span>Record<br>video</span>'; record.addEventListener("click", openVideoRecorder);
    const voice = document.createElement("button"); voice.type = "button"; voice.className = "composer-video-action"; voice.setAttribute("aria-label", "Record voice message"); voice.innerHTML = '<span class="composer-media-glyph" aria-hidden="true">◌</span><span>Voice</span>'; voice.addEventListener("click", openVoiceRecorder);
