...
describe("avatar and profile helpers", () => {
  it("defaults incognito off and uses exact student ID approval RPCs", async () => {
    ...
    await setMtuApprovedPerson(client, "MTU-ABC123", true);
    await setMtuApprovedPerson(client, "MTU-ABC123", false);
    expect(calls).toEqual([
      ["list_mtu_approved_people", undefined],
      ["set_mtu_approved_person", { p_student_id: "MTU-ABC123", p_approved: true }],
      ["set_mtu_approved_person", { p_student_id: "MTU-ABC123", p_approved: false }],
    ]);
  });
