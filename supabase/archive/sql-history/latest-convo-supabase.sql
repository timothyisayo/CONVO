-- CONVO LATEST SUPABASE SQL HANDOFF
-- Run this entire file once in Supabase Dashboard -> SQL Editor.
-- Do not run drizzle/0000_lying_cyclops.sql; that file targets MySQL/Drizzle, not Supabase.
-- The script creates/updates tables, RPCs, RLS, storage rules, Realtime entries, and messaging/group extensions without seeding fake student or message data.
-- Do not manually call auth-bound RPCs from the SQL Editor; the signed-in app calls them after migration.

-- ===== 1. BASE FOUNDATION =====
-- CONVO: COMPLETE SUPABASE SETUP
-- Run this entire file in Supabase Dashboard → SQL Editor.
-- Order: messaging foundation → campus data → profile/storage → directory search.
-- This bundle is composed from the project’s validated rerun-safe handoff files.
...
drop function if exists public.list_mtu_connection_requests() cascade;
...
drop function if exists public.list_mtu_conversations_v2() cascade;
