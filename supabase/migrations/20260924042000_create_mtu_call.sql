...
begin
  ...
  if conversation_kind = 'group' then
    ...
    if p_callee_id is null
      or p_callee_id = auth.uid()
      or not exists (
    ...
    select allow_calls into callee_policy
    from public.mtu_privacy_settings
    ...
    if callee_policy = 'connections'
      and not exists (
        select 1
        from public.connection_requests
        where status = 'accepted'
          and (
            (requester_id = auth.uid() and recipient_id = p_callee_id)
  ...
  if exists (
    select 1
    from public.mtu_call_participants
  ...
  values (
    p_conversation_id,
    auth.uid(),
    case when conversation_kind = 'group' then null else p_callee_id end,
    p_call_type,
    p_room_name
  )
